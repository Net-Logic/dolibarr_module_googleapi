...
/**
 * \file    googleapi/core/ajax/ecmgoogledrivedelete.php
 * \ingroup googleapi
 * \brief   Moves a Google Drive file or folder to the trash, returns a JSON status so the
 *          caller can show the user a real success/error message instead of discarding it.
 */
...
if (!$user->hasRight('googleapi', 'delete')) {
...
if ($fileid) {
	...
	if (is_object($driveservice)) {
		try {
			...
			$response['message'] = $langs->trans("GoogleApiDriveFileDeleted");
		} catch (Exception $e) {
