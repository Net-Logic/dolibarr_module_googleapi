...
/**
 * \file    htdocs/modulebuilder/template/lib/googleapi.lib.php
 * \ingroup googleapi
 * \brief   Library files with common functions for GoogleApi
 */
 ...
 * @return array
 */
function googleapiAdminPrepareHead()
{
	global $langs, $conf;
	...
	$head[$h][0] = dol_buildpath("/googleapi/admin/setup.php", 1);
	$head[$h][1] = $langs->trans("GoogleApiSettings");
	$head[$h][2] = 'settings';
	$h++;

	$head[$h][0] = dol_buildpath('/googleapi/admin/oauthlogintokens.php', 1);
	$head[$h][1] = $langs->trans("GoogleApiTokenManager");
	$head[$h][2] = 'tokengeneration';
	$h++;

	$head[$h][0] = dol_buildpath("/googleapi/admin/about.php", 1);
	$head[$h][1] = $langs->trans("GoogleApiAbout");
	$head[$h][2] = 'about';
	$h++;

	// Show more tabs from modules
	complete_head_from_modules($conf, $langs, null, $head, $h, 'googleapi');
...
function googleapi_complete_label_and_note(&$object, $langs)
{
	...
	if (($object->socid > 0 || (!empty($object->thirdparty->id) && $object->thirdparty->id > 0)) && empty($conf->global->GOOGLEAPI_DISABLE_EVENT_LABEL_INC_SOCIETE)) {
		...
		if ($result > 0) {
			// we add thirdparty name into label
			...
			$urltoelem = '<a href="' . $urlwithroot . '/societe/soc.php?socid=' . $thirdparty->id . '">' . $thirdparty->name . '</a>';
	...
	if (($object->contactid > 0 || (!empty($object->contact->id) && $object->contact->id > 0)) && empty($conf->global->GOOGLEAPI_DISABLE_EVENT_LABEL_INC_CONTACT)) {
		...
		if ($result > 0) {
			// we add contact fullname into label
			...
			$urltoelem = '<a href="' . $urlwithroot . '/contact/card.ph?id=' . $contact->id . '">' . $contact->getFullName($langs, 1) . '</a>';
			$object->note .= "<br>" . $more . "<br>" . $langs->trans("GoogleApiLinkToContact") . ': ' . $urltoelem;
		}
	}
...
function googleapiCreateActioncomm($owner, $type_code, $date_start, $date_end, $label, $description = '', $location = '', $googleapiId = '', $notrigger = 0)
{
	...
	$actioncomm->userownerid = $owner->id;
	//$actioncomm->authorid = $owner->id;
	$transparency = 1;
	// $actioncomm->userassigned[$object->fk_user] = [
	//     'id' => $object->fk_user,
	...
	$actioncomm->note = $description;
	$actioncomm->note_private = $description;
	$actioncomm->contact = null;
	$actioncomm->socid = null;
	$actioncomm->societe = null;

	// context for triggers
	$actioncomm->context['googleapi'] = $googleapiId;
	// ActionComm create
	$resultid = $actioncomm->create($owner, $notrigger);
	if ($resultid > 0) {
		$actioncomm->array_options['options_googleapi_EventId'] = $googleapiId;
		$actioncomm->insertExtraFields();
		dol_syslog("googleapiCreateActioncomm SUCCESS: " . $resultid, LOG_NOTICE);
		// Add Linked object
		//$actioncomm->id = $idaction;
		//$actioncomm->add_object_linked($object->element, $object->id);
	} else {
		dol_syslog("googleapiCreateActioncomm error: " . $actioncomm->errorsToString(), LOG_ERR);
 ...
 * @return  int                 <0 not ok, >0 ok
 */
function googleapiDeleteActioncomm($object)
{
	global $db, $langs;

	dol_syslog("googleapiDeleteActioncomm ", LOG_DEBUG);

	include_once DOL_DOCUMENT_ROOT . '/comm/action/class/actioncomm.class.php';
	$actioncomm = new ActionComm($db);

	// $sql = 'SELECT DISTINCT fk_target';
	// $sql .= ' FROM '.MAIN_DB_PREFIX.'element_element';
	// $sql .= " WHERE sourcetype = '".$db->escape($object->element)."'";
	// $sql .= " AND fk_source = ".$object->id;
	// $sql .= " AND targettype = '".$db->escape($actioncomm->element)."'";
	...
	while ($resql && $obj = $db->fetch_object($resql)) {
		//var_dump($obj);exit;
		setEventMessage('actioncomm delete' . $obj->id);
		$actioncomm = new ActionComm($db);
		$result = $actioncomm->fetch($obj->id);
		$actioncomm->fetch_optionals();
		$actioncomm->fetch_userassigned();
		$actioncomm->context['googleapi'] = $object->array_options['options_googleapi_EventId'];
		$actioncomm->oldcopy = clone $actioncomm;
		if ($result > 0 /*&& $actioncomm->label == $holiday_label*/) {
			// On supprime l'agenda
			$ret = $actioncomm->delete();
			if ($ret > 0) {
				// Linked object
				// $ret = $actioncomm->deleteObjectLinked(null, null, null, null, $rowid);
				// if ($ret < 0) {
				//     dol_syslog("googleapiDeleteActioncomm Error delete link: " . $actioncomm->errorsToString(), LOG_ERR);
				//     setEventMessages($actioncomm->error, $actioncomm->errors, 'errors');
				//     return -1;
				// }
			} else {
				dol_syslog("googleapiDeleteActioncomm Error delete: " . $actioncomm->errorsToString(), LOG_ERR);
				setEventMessages($actioncomm->error, $actioncomm->errors, 'errors');
				return -1;
			}
		}
 ...
 * @return  boolean
 */
function googleapiFindBadUtf8($string)
{
	$regex = '([\x00-\x7F]';
 ...
 * @return string
 */
function googleapiRemoveEmoji($text)
{
...
/**
 * Function to verify signature of token
 * https://oa.dnc.global/web/-Jeton-d-identite-JSON-Web-Token-JWT-.html
 * @param   string  $signature  signature
 * @param   string  $input      input used to generate signature
 * @param   string  $key        public key given by oauth issuer
 * @param   string  $algo       algo name used to generate signature
 * @return  bool
 * @throw
 */
function verifyGoogleApiSignature($signature, $input, $key, $algo = 'HS256')
{
